...
extend(ChartInternal_ChartInternal.prototype, {
  ...
  updateLine: function updateLine(durationForExit) {
    ...
    $$.mainLine = $$.main.selectAll(".".concat(config_classes.lines)).selectAll(".".concat(config_classes.line)).data($$.lineData.bind($$)), $$.mainLine.exit().transition().duration(durationForExit).style("opacity", "0").remove(), $$.mainLine = $$.mainLine.enter().append("path").attr("class", function (d) {
      return "".concat($$.classLine.call($$, d), " ").concat($$.extraLineClasses(d) || "");
...
extend(ChartInternal_ChartInternal.prototype, {
  ...
  generatePoint: function generatePoint() {
    var $$ = this,
        config = $$.config,
        patternConfig = notEmpty(config.point_pattern) ? config.point_pattern : [config.point_type],
        patterns = patternConfig.map(function (point) {
      return $$.hasValidPointType(point) ? [null, $$[point]] : $$.hasValidPointDrawMethods(point) ? [null, point] : ["custom", point];
    }),
        idPatternMap = {},
        keysInMap = 0,
        getPatternForId = function (id) {
      if (idPatternMap[id]) return idPatternMap[id];
      var pattern = patterns[keysInMap % patterns.length];
      return keysInMap += 1, idPatternMap[id] = pattern, pattern;
    };
    ...
    return function (method, context) {
      ...
      return function (d) {
        var _point$method,
            ...
            _getPatternForId = getPatternForId(id),
            _getPatternForId2 = slicedToArray_default()(_getPatternForId, 2),
            patternType = _getPatternForId2[0],
            point = _getPatternForId2[1];

        if (patternType === "custom") {
          var pointId = "".concat($$.datetimeId, "-point-").concat(id),
              pointFromDefs = $$.pointFromDefs(pointId);

          if (pointFromDefs.size() < 1 && $$.insertPointInfoDefs(point, pointId), method === "create") {
            var _$$$custom$create;

            return (_$$$custom$create = $$.custom.create).call.apply(_$$$custom$create, [context, element, pointId].concat(args));
          }

          if (method === "update") {
            var _$$$custom$update;

            return (_$$$custom$update = $$.custom.update).call.apply(_$$$custom$update, [context, element].concat(args));
          }
        }

        return (_point$method = point[method]).call.apply(_point$method, [context, element].concat(args));
      };
    };
  },
...
extend(ChartInternal_ChartInternal.prototype, {
  ...
  initZoomBehaviour: function initZoomBehaviour() {
    ...
    $$.zoomBehaviour = Object(external_commonjs_d3_drag_commonjs2_d3_drag_amd_d3_drag_root_d3_["drag"])().clickDistance(4).on("start", function () {
      ...
      if ($$.setDragStatus(!1), zoomRect.attr(prop.axis, 0).attr(prop.attr, 0), start > end && (_ref = [end, start], start = _ref[0], end = _ref[1], _ref), start < 0 && (end += Math.abs(start), start = 0), start !== end) $$.api.zoom([start, end].map(function (v) {
      ...
      })), $$.onZoomEnd();else if ($$.isMultipleX()) $$.clickHandlerForMultipleXS.call(this, $$);else {
        ...
        $$.clickHandlerForSingleX.call(target, Object(external_commonjs_d3_selection_commonjs2_d3_selection_amd_d3_selection_root_d3_["select"])(target).datum(), $$);
      }
    });
  },
